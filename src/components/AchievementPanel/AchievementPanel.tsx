  ...
  return (
    <div className="achievement-panel" style={{ minWidth: 600, background: 'linear-gradient(135deg, #23304a 0%, #2d3e5e 100%)', borderRadius: 20, padding: '36px 32px', color: '#fff', boxShadow: '0 8px 32px rgba(0,0,0,0.18)', position: 'relative' }}>
      ...
      <div style={{ maxHeight: 340, overflowY: 'auto', paddingRight: 8 }}>
        {allList.length === 0 ? (
          <div style={{ color: '#888', textAlign: 'center', fontSize: 18, marginTop: 40 }}>No badges yet</div>
        ) : (
              ...
              return (
                <div
                  ...
                  style={{
                    display: 'flex',
                    alignItems: 'center',
                    background: isUnlocked ? 'rgba(255,255,255,0.98)' : 'rgba(220,230,255,0.7)',
                    borderRadius: 14,
                    padding: '18px 20px',
                    boxShadow: isUnlocked ? '0 2px 12px rgba(0,0,0,0.10)' : 'none',
                  ...
                  onMouseEnter={e => {
                    if (isUnlocked) e.currentTarget.style.boxShadow = '0 4px 18px rgba(0,0,0,0.18)';
                    if (isUnlocked) e.currentTarget.style.transform = 'scale(1.03)';
                  }}
                  onMouseLeave={e => {
                    if (isUnlocked) e.currentTarget.style.boxShadow = '0 2px 12px rgba(0,0,0,0.10)';
